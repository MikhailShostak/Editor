...
#include "Core/Settings/SettingsEntry.gen.h"
#include "Core/Extensions/ExtensionsView.gen.h"
#include "Core/Settings/DeveloperLibraryView.gen.h"
#include "Core/Settings/Toolbar/ToolbarSettingsView.gen.h"
#include "Core/Types/TypesView.gen.h"
#include "Core/Files/EditableFile.gen.h"

#include "Core/Tasks/TasksProgressView.gen.h"

#include "Core/OneStudioFlow.gen.h"

#include "Core/EnvironmentVariables/EnvironmentVariablesView.gen.h"
...
#include "Core/Tasks/TasksWidget.gen.h"
...
void ApplicationManager::indexLibrary(OneStudio::Task &task)
{
    ...
    if (libraryDirectory.isExists())
    {
        ...
        for (const APathInfo& libraryEntry : entries)
        {
            ...
            task.log.push_back(Str::Format(fmt::runtime("Indexing library {}"), libraryEntry.name()));
            if (!libraryRoot.isExists())
            {
                task.log.push_back(Str::Format(fmt::runtime("    Warning: Library {} doesn't have a current version or content"), libraryEntry.name()));
                continue;
            }
            ...
            {
                task.log.push_back(Str::Format(fmt::runtime("    {}"), libraryFile.path().toUnixPathString()));
...
void ApplicationManager::onLoad()
{
    ...
    task->action = [](OneStudio::Task &task) -> void { task.log.push_back(Str::Format(fmt::runtime("{}"), "Init!")); };
