...
inline AError PluginLibrary<PluginInterface>::load()
{
    ...
    for (auto &plugin : plugins)
    {
        ...
        for (auto &dependence : plugin->getInstance()->getDependencies())
        {
            ...
            {
                return AError(1, fmt::format(fmt::runtime(FormatString), plugin->getPackage(), dependence.first, NotFoundReson));
            }

            auto context = (*dependentPlugin)->getInstance()->getContext();
            if (!context)
            {
                return AError(2, fmt::format(fmt::runtime(FormatString), plugin->getPackage(), dependence.first, NoContextReson));
            }
